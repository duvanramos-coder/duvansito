...
const gasPath = path.join(__dirname, 'gas_output');

if (!fs.existsSync(gasPath)) {
  fs.mkdirSync(gasPath);
}
...
if (cssFile) {
  ...
  fs.writeFileSync(path.join(gasPath, 'styles.html'), `<style>\n${cssContent}\n</style>`);
  console.log('Generated styles.html');
}

if (jsFile) {
  const jsContent = fs.readFileSync(path.join(assetsPath, jsFile), 'utf8');
  fs.writeFileSync(path.join(gasPath, 'scripts.html'), `<script>\n${jsContent}\n</script>`);
  console.log('Generated scripts.html');
}

// Generate index.html for GAS
...
<html>
  <head>
    ...
    <title>Dashboard Pro - People BPO</title>
    <?!= include('styles'); ?>
  </head>
  <body>
    <div id="root"></div>
...
fs.writeFileSync(path.join(gasPath, 'index.html'), indexHtml);
console.log('Generated index.html');

// Create a copy of the original .gs logic
const gsLogic = `function doGet() {
  try {
...
fs.writeFileSync(path.join(gasPath, 'Code.gs'), gsLogic);
